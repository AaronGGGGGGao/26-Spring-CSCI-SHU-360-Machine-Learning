"""
Stage1-oriented 3-day XGBoost model.

Compared with the clean 3-day baseline, this version changes three things:
  - richer short-horizon feature set
  - recency-weighted training as an option
  - broader portfolio construction search
"""
...
DEFAULT_RECENCY_HALFLIVES = ["none", "10", "20"]
MODEL_CONFIGS = [
    {
        "name": "base",
        "n_estimators": 500,
...
def train_model(train_df: pd.DataFrame, val_df: pd.DataFrame, config: dict, half_life: int | None) -> xgb.XGBRegressor:
    ...
    train_w = sample_weights_from_dates(train_df["date"], half_life)
    val_w = sample_weights_from_dates(val_df["date"], half_life)
    model.fit(
...
def evaluate_model(model: xgb.XGBRegressor, frame: pd.DataFrame, index_df: pd.DataFrame, top_k: int, weight_method: str) -> dict[str, float]:
...
def select_model_and_portfolio(train_df, val_df, index_df, configs, top_ks, methods, halflives):
    leaderboard = []
    best_key = None
    best = None
    for half_life in halflives:
        for config in configs:
            model = train_model(train_df, val_df, config, half_life)
            for top_k in top_ks:
                for method in methods:
                    ...
                    if best_key is None or key > best_key:
                        best_key = key
                        best = (model, config["name"], int(top_k), method, half_life, metrics)
...
def main() -> None:
    ...
    p.add_argument("--recency-halflives", default="none,10,20")
    ...
    print(">> Building stage1 XGBoost features")
    ...
    model, best_config, best_top_k, best_method, best_half_life, val_metrics, leaderboard = select_model_and_portfolio(
        train_df, val_df, index_df, MODEL_CONFIGS, top_ks, methods, halflives
    )
    half_life_label = "none" if best_half_life is None else str(best_half_life)
    print(f"   selected config/top_k/weight/half_life: {best_config} / {best_top_k} / {best_method} / {half_life_label}")
