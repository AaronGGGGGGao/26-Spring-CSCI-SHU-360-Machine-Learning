"""
3-day Ridge model with enhanced feature engineering and tuned weight construction.
"""
...
from ridge.day5_stage2_enhanced.features import (  # noqa: E402
    FEATURE_COLUMNS,
    FORWARD_HORIZON,
    TARGET_COLUMN,
    build_features,
...
DEFAULT_TOP_KS = [30, 40, 50, 60, 80]
DEFAULT_ALPHAS = [0.01, 0.1, 1.0, 10.0, 100.0]
DEFAULT_WEIGHT_METHODS = ["equal", "rank", "softmax", "score", "rank_inv_vol"]
...
def build_portfolio_custom(daily: pd.DataFrame, top_k: int, method: str) -> pd.Series:
    ...
    if method == "equal":
        ...
        risk = chosen["vol_10d"].replace(0, np.nan).fillna(chosen["vol_10d"].median())
        risk = risk.fillna(1.0)
        raw = ranks / risk
        w = raw / raw.sum()
    else:
...
def select_model_and_portfolio(train_df: pd.DataFrame, val_df: pd.DataFrame, index_df: pd.DataFrame, alphas: list[float], top_ks: list[int], methods: list[str]):
    leaderboard = []
    best_key = None
    best = None
    for alpha in alphas:
        ...
        for top_k in top_ks:
            for method in methods:
                ...
                row = {"alpha": float(alpha), "top_k": int(top_k), "weight_method": method, **metrics}
                ...
                if best_key is None or key > best_key:
                    best_key = key
                    best = (model, float(alpha), int(top_k), method, metrics)
...
def main() -> None:
    ...
    print(">> Training Ridge 5d enhanced")
    ...
    if "mean_excess_return" in val_metrics:
        print("   validation mean 5d returns "
