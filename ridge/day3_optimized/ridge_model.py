"""
Optimized 3-day Ridge model for stage1.

Key changes relative to earlier 3-day versions:
  - train on 3-day excess return rather than raw return
  - use richer short-horizon / market-relative features
  - tune recency weighting, top_k, and weight method jointly
"""
...
from ridge.day3_optimized.features import (  # noqa: E402
    FEATURE_COLUMNS,
    FORWARD_HORIZON,
    RAW_RETURN_COLUMN,
    TARGET_COLUMN,
...
DEFAULT_RECENCY_HALFLIVES = ["none", "10", "20", "40"]
...
def period_excess_return(
    ...
    for d, daily in scored.groupby("date"):
        ...
        realized = daily.set_index("stock_code")[RAW_RETURN_COLUMN].reindex(weights.index)
...
def select_model_and_portfolio(
    ...
    for half_life in halflives:
        for alpha in alphas:
            ...
            model = fit_model(model, train_df, half_life)
            for top_k in top_ks:
...
def main() -> None:
    ...
    p.add_argument("--top-ks", default="30,35,40,50,60")
    ...
    print(">> Building optimized 3-day features")
    ...
    print(">> Training optimized Ridge 3d")
