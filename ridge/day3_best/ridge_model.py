"""
Best-effort stage1 Ridge model.

This version keeps the stronger day3_enhanced feature set and expands only the
model-selection side: recency weighting and a broader weight-construction grid.
"""
...
from baseline.baseline_xgboost import EMBARGO_DAYS, MAX_WEIGHT, MIN_STOCKS, VAL_DAYS, rank_ic  # noqa: E402
from baseline.paths import DATA_DIR  # noqa: E402
from ridge.day3_enhanced.features import (  # noqa: E402
    FEATURE_COLUMNS,
    FORWARD_HORIZON,
    TARGET_COLUMN,
    build_features,
...
DEFAULT_TOP_KS = [30, 35, 40, 45, 50, 60]
DEFAULT_ALPHAS = [0.001, 0.01, 0.1, 1.0, 3.0, 10.0, 30.0, 100.0]
...
def parse_halflives(text: str) -> list[int | None]:
    out: list[int | None] = []
...
def sample_weights_from_dates(dates: pd.Series, half_life: int | None) -> np.ndarray:
    if half_life is None:
        return np.ones(len(dates), dtype=float)
    uniq = pd.Index(pd.to_datetime(dates).sort_values().unique())
    pos = pd.Series(np.arange(len(uniq), dtype=float), index=uniq)
    mapped = pd.to_datetime(dates).map(pos)
    age = mapped.max() - mapped
    return np.power(0.5, age / float(half_life)).to_numpy(dtype=float)


def fit_model(model: Pipeline, train_df: pd.DataFrame, half_life: int | None) -> Pipeline:
    sample_weight = sample_weights_from_dates(train_df["date"], half_life)
    model.fit(train_df[FEATURE_COLUMNS], train_df[TARGET_COLUMN], ridge__sample_weight=sample_weight)
    return model
...
def build_portfolio_custom(daily: pd.DataFrame, top_k: int, method: str) -> pd.Series:
    ...
    risk = chosen["vol_10d"].replace(0, np.nan)
    ...
    w = _cap_and_redistribute(w.astype(float))
...
def select_model_and_portfolio(train_df, val_df, index_df, alphas, top_ks, methods, halflives):
    leaderboard = []
    best_key = None
    best = None
    for half_life in halflives:
        for alpha in alphas:
            model = fit_model(make_model(alpha), train_df, half_life)
            for top_k in top_ks:
                for method in methods:
                    ...
                    row = {
                        ...
                        "recency_half_life": None if half_life is None else int(half_life),
                        **metrics,
                    }
                    ...
                    if best_key is None or key > best_key:
                        best_key = key
                        best = (model, float(alpha), int(top_k), method, half_life, metrics)
...
def main() -> None:
    ...
    p.add_argument("--alphas", default="0.001,0.01,0.1,1,3,10,30,100")
    p.add_argument("--top-ks", default="30,35,40,45,50,60")
    p.add_argument("--weight-methods", default="equal,rank,softmax,score,score_sq,score_inv_vol,rank_inv_vol")
    p.add_argument("--recency-halflives", default="none,10,20,40")
    ...
    halflives = parse_halflives(args.recency_halflives)
    ...
    print(">> Building best-effort 3-day features")
    ...
    print(">> Training best-effort Ridge 3d")
    model, best_alpha, best_top_k, best_method, best_half_life, val_metrics, leaderboard = select_model_and_portfolio(
        train_df, val_df, index_df, alphas, top_ks, methods, halflives
    )
    half_life_label = "none" if best_half_life is None else str(best_half_life)
    print(f"   selected alpha/top_k/weight/half_life: {best_alpha:g} / {best_top_k} / {best_method} / {half_life_label}")
    ...
    if args.json_out:
        payload = {
            ...
            "selected_recency_half_life": best_half_life,
            "alphas": alphas,
            "top_ks": top_ks,
            "weight_methods": methods,
            "recency_halflives": [None if h is None else int(h) for h in halflives],
