"""
Self-test for the 5-day Ridge model using a train/validation/test time split.
"""
...
from baseline.features import FORWARD_HORIZON, build_features, training_frame  # noqa: E402
from baseline.paths import DATA_DIR  # noqa: E402
from ridge.day5.ridge_model import DEFAULT_TOP_K, evaluate_model, parse_alphas, select_alpha  # noqa: E402
...
def build_splits(panel: pd.DataFrame, as_of: str | None = None) -> dict[str, pd.DataFrame | pd.Timestamp]:
...
def main() -> None:
    ...
    test_metrics = evaluate_model(model, test_df, index_df, top_k=args.top_k)
    ...
    if "mean_excess_return" in test_metrics:
        ...
        print(
            f"   test positive excess rate: {test_metrics['positive_excess_rate']*100:.1f}% "
            f"over {int(test_metrics['n_dates'])} dates"
        )
