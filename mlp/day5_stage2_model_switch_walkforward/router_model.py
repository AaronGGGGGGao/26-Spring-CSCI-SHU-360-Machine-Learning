...
def main() -> None:
    ...
    print(">> Fitting recent-window + style-dynamic branch models")
    ...
    print(
        "   recent branch: "
        f"{dev_ctx['recent_selection']['lookback']} / "
        f"{dev_ctx['recent_selection']['config']} / "
        f"{dev_ctx['recent_selection']['top_k']} / "
        f"{dev_ctx['recent_selection']['weight_method']}"
    )
    print(
        "   style branch: "
    ...
    if args.json_out:
        payload = {
            "prediction": meta,
            "recent_selection": dev_ctx["recent_selection"],
