"""
Walk-forward robustness evaluation for the Stage 2 5-day style-dynamic MLP.
"""
...
try:  # noqa: E402
    ...
    from features import FORWARD_HORIZON, build_features, training_frame
    from mlp_model import ALLOCATION_POLICIES, MLP_CONFIGS, evaluate_model, parse_int_list, select_model_and_policy
...
def main() -> None:
    ...
    policy_names = _parse_policies(args.policies)
    ...
    print(">> Stage 2 5-day style-dynamic walk-forward windows")
    for spec in windows:
        print(
    ...
    for spec in windows:
        ...
        print(f">> Training style-dynamic window {spec['window_id']}")
        model, best_config, best_half_life, best_policy, val_metrics, leaderboard = select_model_and_policy(
            train_df, val_df, index_df, MLP_CONFIGS, half_lives, policy_names
        )
        ...
        results.append(
            {
                ...
                "selected_half_life": int(best_half_life),
    ...
    print(">> Aggregate Stage 2 5-day style-dynamic walk-forward result")
    ...
    if args.json_out:
        out = {
            "dataset": "data",
            "model_family": "mlp_day5_stage2_style_dynamic",
            "methodology": {
                "type": "walk_forward",
                "windows": args.windows,
                "val_days": args.val_days,
        ...
        print(f">> Wrote style-dynamic walk-forward summary to {out_path}")
