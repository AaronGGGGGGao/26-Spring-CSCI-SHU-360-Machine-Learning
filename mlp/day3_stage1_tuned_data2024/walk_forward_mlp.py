"""
Walk-forward robustness evaluation for the tuned stage1 MLP on ./data_2024.

This script does not replace the canonical single self-test split. It adds a
separate, more stable multi-window evaluation on the extended-history dataset.

Methodology
-----------
- same model family and feature set as the current stage1 winner
- same 3-day horizon
- same teacher-imposed portfolio constraints
- expanding train window
- per-window validation and test blocks with embargo in between
...
def main() -> None:
    ...
    p.add_argument("--prices", default=str(DATA_2024_DIR / "prices.parquet"))
    p.add_argument("--index", default=str(DATA_2024_DIR / "index.parquet"))
    ...
    for spec in windows:
        ...
        print(
            f"   selected config/top_k/weight: {best_config} / {best_top_k} / {best_method}"
        )
    ...
    if args.json_out:
        out = {
            "dataset": "data_2024",
            "model_family": "mlp_day3_stage1_tuned",
            "methodology": {
                "type": "walk_forward",
                "windows": args.windows,
                "val_days": args.val_days,
