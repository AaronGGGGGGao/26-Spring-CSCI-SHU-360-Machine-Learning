"""
Stage 2 5-day score blend of recent-window MLP and style-dynamic MLP.

The two branch models are trained independently, then their daily cross-sectional
scores are z-scored and blended. Portfolio construction is applied once to the
blended score. This tests complementarity without a heavier regime router.
"""
...
def _branch_frames(recent_panel: pd.DataFrame, style_panel: pd.DataFrame, bounds: dict, include_test: bool = False) -> dict:
...
def train_branch_models(frames: dict, index_df: pd.DataFrame, top_ks: list[int], methods: list[str], lookbacks: list[str], half_lives: list[int], style_policies: list[str]) -> dict:
    ...
    return {"recent": recent, "style": style}


def _score_frame(recent_model, style_model, recent_frame: pd.DataFrame, style_frame: pd.DataFrame) -> pd.DataFrame:
...
def _period_excess_return(scored: pd.DataFrame, index_df: pd.DataFrame, alpha: float, top_k: int, method: str):
    frame = scored.copy()
    frame["score"] = alpha * frame["recent_z"] + (1.0 - alpha) * frame["style_z"]
...
def evaluate_blend(scored: pd.DataFrame, index_df: pd.DataFrame, alpha: float, top_k: int, method: str) -> dict[str, float]:
    score = alpha * scored["recent_z"] + (1.0 - alpha) * scored["style_z"]
    ic = rank_ic(scored[TARGET_COLUMN].to_numpy(), score.to_numpy(), scored["date"].to_numpy())
    _, bt = _period_excess_return(scored, index_df, alpha=alpha, top_k=top_k, method=method)
...
def select_blend(scored_val: pd.DataFrame, index_df: pd.DataFrame, alphas: list[float], top_ks: list[int], methods: list[str]):
    leaderboard = []
    best_key = None
    best = None
    for alpha in alphas:
        for top_k in top_ks:
            for method in methods:
                metrics = evaluate_blend(scored_val, index_df, alpha=alpha, top_k=top_k, method=method)
                row = {"blend_recent_weight": float(alpha), "top_k": int(top_k), "weight_method": method, **metrics}
                ...
                if best_key is None or key > best_key:
                    best_key = key
                    best = (float(alpha), int(top_k), method, metrics)
    if best is None:
...
def build_prediction(
    ...
    scored = _score_frame(recent_model, style_model, recent_pred, style_pred)
    scored["score"] = alpha * scored["recent_z"] + (1.0 - alpha) * scored["style_z"]
...
def main() -> None:
    ...
    print(">> Building recent-window and style-dynamic panels")
    ...
    alpha, best_top_k, best_method, val_metrics, blend_leaderboard = select_blend(
