"""
Stage 2 5-day correlation-diversified portfolio-level ensemble.
...
averaging coefficient with a stability-aware validation objective. This
variant adds a low-degree-of-freedom correlation diversification layer after
the child portfolios are combined.
"""
...
DEFAULT_CORR_WINDOWS = [20, 40, 60]
DEFAULT_CORR_PENALTIES = [0.0, 0.25, 0.50, 0.75]
DEFAULT_DIVERSE_NAMES = [0, 30, 35, 40]
...
def build_return_panel(prices: pd.DataFrame) -> pd.DataFrame:
    panel = prices[["date", "stock_code", "close"]].copy()
    panel["date"] = pd.to_datetime(panel["date"])
    panel["stock_code"] = panel["stock_code"].astype(str).str.zfill(6)
    panel = panel.sort_values(["stock_code", "date"])
    panel["ret_1d"] = panel.groupby("stock_code")["close"].pct_change()
    return panel.pivot(index="date", columns="stock_code", values="ret_1d").sort_index()


def _cap_and_normalize(weights: pd.Series) -> pd.Series:
    weights = weights[weights > 1e-12].astype(float)
    if len(weights) < MIN_STOCKS:
        raise RuntimeError(f"diversified portfolio produced only {len(weights)} names")
    ...
    for _ in range(20):
        ...
        under = ~over
        if not bool(under.any()) or weights.loc[under].sum() <= 0:
            break
        weights.loc[under] += excess * weights.loc[under] / weights.loc[under].sum()
    weights = weights / weights.sum()
    if weights.max() > MAX_WEIGHT + 1e-8:
        raise RuntimeError(f"diversified portfolio violates {MAX_WEIGHT:.0%} cap")
    return weights.sort_values(ascending=False)


def _corr_matrix_for_date(
    return_panel: pd.DataFrame,
    date: pd.Timestamp,
    candidates: list[str],
    corr_window: int,
) -> pd.DataFrame:
    hist = return_panel.loc[return_panel.index < pd.Timestamp(date), candidates].tail(corr_window)
    min_periods = max(8, min(corr_window // 3, len(hist)))
    corr = hist.corr(min_periods=min_periods).abs()
    corr = corr.reindex(index=candidates, columns=candidates).fillna(0.0)
    arr = np.array(corr, dtype=float, copy=True)
    np.fill_diagonal(arr, 0.0)
    return pd.DataFrame(arr, index=corr.index, columns=corr.columns)


def diversify_weight_dict(
    ...
    base = pd.Series(base_portfolio, dtype=float).sort_values(ascending=False)
    base = base[base > 1e-12]
    if corr_penalty <= 0.0 and target_names <= 0:
        return _weights_to_dict(_cap_and_normalize(base))

    if target_names <= 0 or target_names >= len(base):
        target_names = len(base)
    target_names = max(MIN_STOCKS, min(int(target_names), len(base)))
    candidates = list(base.index)
    corr = _corr_matrix_for_date(return_panel, date, candidates, corr_window)
    base_score = base / max(float(base.max()), 1e-12)

    selected = [str(base.index[0])]
    remaining = set(candidates) - set(selected)
    while len(selected) < target_names and remaining:
        rem = sorted(remaining)
        avg_corr = corr.loc[rem, selected].mean(axis=1).fillna(0.0)
        score = base_score.loc[rem] - float(corr_penalty) * avg_corr
        best = str(score.sort_values(ascending=False).index[0])
        selected.append(best)
        remaining.remove(best)

    diversified = _cap_and_normalize(base.loc[selected])
    return _weights_to_dict(diversified)
...
def combine_portfolios(
    ...
    out = {}
    for date in dates:
        combined = combine_weight_dicts(style_portfolios[date], feature_portfolios[date], style_weight)
        if return_panel is not None:
            combined = diversify_weight_dict(
                combined,
                return_panel,
                date,
...
def select_portfolio_ensemble(
    ...
    corr_penalties: list[float],
    diverse_names: list[int],
    vol_penalty: float,
    downside_penalty: float,
    positive_bonus: float,
    ...
    for style_weight in portfolio_weights:
        for corr_window in corr_windows:
            for corr_penalty in corr_penalties:
                for target_names in diverse_names:
                    if corr_penalty <= 0.0 and target_names > 0:
                        continue
                    combined = combine_portfolios(
                        style_portfolios,
                        feature_portfolios,
                        style_weight,
                    ...
                    params = {
                        "style_recent_portfolio_weight": float(style_weight),
                        "corr_window": int(corr_window),
                        "corr_penalty": float(corr_penalty),
                        "target_names": int(target_names),
                    }
                    row = {**params, "objective": objective, **metrics}
                    ...
                    if best_key is None or key > best_key:
                        best_key = key
                        best = (params, {**metrics, "objective": objective})
    if best is None:
...
def build_prediction(
    ...
    combined = combine_portfolios(
        style_ports,
        feature_ports,
        ensemble_params["style_recent_portfolio_weight"],
        return_panel=return_panel,
        corr_window=int(ensemble_params["corr_window"]),
        corr_penalty=float(ensemble_params["corr_penalty"]),
        target_names=int(ensemble_params["target_names"]),
    )
...
def main() -> None:
    ...
    p.add_argument("--corr-windows", default="20,40,60")
    p.add_argument("--corr-penalties", default="0.0,0.25,0.50,0.75")
    p.add_argument("--diverse-names", default="0,30,35,40")
    ...
    corr_windows = parse_int_list(args.corr_windows)
    corr_penalties = parse_float_list(args.corr_penalties)
    diverse_names = parse_int_list(args.diverse_names)
    ...
    print(
        "   selected params: "
        f"style_weight={ensemble_weight['style_recent_portfolio_weight']:.2f}, "
        f"corr_window={ensemble_weight['corr_window']}, "
        f"corr_penalty={ensemble_weight['corr_penalty']:.2f}, "
        f"target_names={ensemble_weight['target_names']}"
    )
    print(
    ...
    if args.json_out:
        payload = {
            ...
            "selected_ensemble_params": ensemble_weight,
            "style_recent": {
