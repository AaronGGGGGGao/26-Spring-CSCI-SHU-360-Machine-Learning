"""
Robust self-test for the tuned stage1 3-day MLP model.

Compared with the canonical self-test, this version strengthens the evidence by:

- using a longer validation block
- using a longer test block
- keeping the same embargo logic
- keeping the same model, features, and portfolio construction search

This is intended as a stricter evaluation protocol, not as a replacement for
the canonical split used in previous comparisons.
"""
...
def main() -> None:
    ...
    print(">> Training tuned MLP (robust split)")
