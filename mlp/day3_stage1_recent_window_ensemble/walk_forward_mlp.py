"""
Walk-forward evaluation for the recent-window rank-ensemble MLP.
"""
...
from mlp.day3_stage1_tuned.mlp_model import MLP_CONFIGS, parse_int_list, parse_str_list  # noqa: E402
...
def main() -> None:
    ...
    for spec in windows:
        ...
        test_metrics = evaluate_ensemble(models, test_df, index_df, top_k=best_top_k, weight_method=best_method)

        print(f"   selected config/top_k/weight: {best_config} / {best_top_k} / {best_method}")
        print(f"   selected lookbacks: {[m['lookback'] for m in model_meta if not m.get('skipped')]}")
    ...
    if args.json_out:
        out = {
            "dataset": "data",
            "model_family": "mlp_day3_stage1_recent_window_ensemble",
            "methodology": {
                "type": "walk_forward",
                "windows": args.windows,
                "val_days": args.val_days,
