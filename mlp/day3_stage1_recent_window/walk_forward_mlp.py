"""
Walk-forward robustness evaluation for the current best recent-window stage1 MLP.

This extends the existing single-split self-tests with multiple rolling
time windows, following the instructor guidance that single-window IC and
single-window excess return are noisy.
"""
...
try:  # noqa: E402
    ...
    from .mlp_model import parse_lookbacks, select_with_lookback
...
def main() -> None:
    ...
    if args.json_out:
        out = {
            "dataset": "data",
            "model_family": "mlp_day3_stage1_recent_window",
            "methodology": {
                "type": "walk_forward",
                "windows": args.windows,
                "val_days": args.val_days,
