"""
Canonical self-test for the stage1 model-switch branch.
"""
...
def main() -> None:
    ...
    result = fit_self_test_models(prices, index_df, val_days=10, test_days=10, as_of=args.as_of)
    ...
    print(">> Training model-switch branch")
    print(
        "   tuned branch: "
        f"{result['tuned_selection']['config']} / "
        f"{result['tuned_selection']['top_k']} / "
        f"{result['tuned_selection']['weight_method']}"
    )
    print(
        "   style branch: "
    ...
    print(
        ...
        f"{result['router_selection']['validation']['mean_portfolio_return']*100:+.3f}% / "
        f"{result['router_selection']['validation']['mean_benchmark_return']*100:+.3f}% / "
    ...
    if args.json_out:
        payload = {
            ...
            "tuned_selection": result["tuned_selection"],
