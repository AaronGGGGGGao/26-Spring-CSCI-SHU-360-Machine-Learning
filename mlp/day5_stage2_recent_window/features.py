"""
Feature engineering for the Stage 2 5-day recent-window MLP.

This branch keeps the strongest Stage 1 public-price factor design but changes
the supervised target and benchmark horizon from 3 trading days to 5 trading
days. It is intentionally independent from the Stage 1 feature module so the
5-day target cannot accidentally fall back to target_3d.
"""
...
RAW_RETURN_COLUMN = "target_5d"
EXCESS_TARGET_COLUMN = "target_excess_5d"
FORWARD_HORIZON = 5
...
FEATURE_COLUMNS = [
    ...
    "relative_strength_3_10", "relative_strength_5_10", "relative_strength_1_5",
    "intraday_ret", "gap_1d", "range_1d", "range_vs_10d", "close_pos_in_range",
    "vol_3d", "vol_5d", "vol_10d", "vol_20d", "vol_ratio_5_20", "downside_vol_10d",
    ...
    "beta_20d", "idio_vol_20d", "idio_mom_5d",
    "excess_ret_1d_rank", "excess_ret_5d_rank", "relative_strength_5_10_rank",
...
def _market_relative_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["idio_mom_5d"] = resid.rolling(5).sum()
    return df
...
def build_features(prices: pd.DataFrame, index_df: pd.DataFrame) -> pd.DataFrame:
    ...
    index_panel["index_target_5d"] = index_panel["close"].shift(-FORWARD_HORIZON) / index_panel["close"] - 1.0
    ...
    panel["relative_strength_5_10"] = panel["excess_ret_5d"] - panel["excess_ret_10d"]
    panel["relative_strength_1_5"] = panel["excess_ret_1d"] - panel["excess_ret_5d"]
    panel[EXCESS_TARGET_COLUMN] = panel[RAW_RETURN_COLUMN] - panel["index_target_5d"]
