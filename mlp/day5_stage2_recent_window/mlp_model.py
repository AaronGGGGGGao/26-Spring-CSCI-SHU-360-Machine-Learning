...
This is a new branch for the 5-trading-day Stage 2 horizon. It keeps the main
idea that worked in Stage 1, recent-window training, but all targets,
benchmark returns, split cutoffs, and printed metrics use a 5-day horizon.
"""
...
def main() -> None:
    ...
    print(">> Building 5-day recent-window feature panel")
    ...
    print(">> Predicting Stage 2 portfolio")
    pred_df = prediction_frame(panel, as_of=args.as_of).copy()
    if pred_df.empty:
        raise RuntimeError(f"No prediction rows available for as_of={args.as_of}.")
